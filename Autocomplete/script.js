...
autoCompleteInputTag.addEventListener("keyup", (event) => {
  if (event.key === "ArrowUp" || 
      event.key === "ArrowDown" || 
      event.key === "Enter") {
        navigateSelectProduct(event.key);
        return;
      }
  //part One
...
const navigateSelectProduct = (key) => {
  ...
  if (key == "ArrowDown") {
    ...
    selectIndex += 1;
    const selectedItemContainer = selectedFnc(selectIndex);

    if (selectIndex > 0) {
      deselectFnc();
    }
    selectedItemContainer.classList.add("selected");
  }else if(key === "ArrowUp") {
    if (selectIndex === -1){
      return;
  }
  if (selectIndex === 0){
    ...
    const selectedProduct = selectedFnc(selectIndex);
    containerDiv.style.display = "none";
    enteredProduct(selectedProduct);

  }
};
const enteredProduct = (enteredElement) => {
  const newDiv = enteredElement;
  newDiv.classList.add("selectedItem");
  bodyTag.append(newDiv);

}
const selectedFnc = () => {
  let selectedId = filteredProducts[selectIndex].id.toString();
    const selectedItemContainer = document.getElementById(selectedId);
    selectedItemContainer.style.border = "1px solid aliceblue";
    selectedItemContainer.style.outline = "outset";
    return selectedItemContainer;
};
const deselectFnc = () => {
  const selectedTag = document.getElementsByClassName("selected")[0];
  selectedTag.style.border = "none";
  selectedTag.style.outline = "none";
